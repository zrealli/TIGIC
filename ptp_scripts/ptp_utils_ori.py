...
def register_attention_control(model, controller, inject_bg=False, pseudo_cross=False): 
            ...
            if inject or inject_bg and is_cross == False:
                ...
                if layernum >= -1 :
                    if place_in_unet == 'down':
                        ...
                        if inject_bg:
                            sim[h:] = controller_for_inject[0].attention_store['up_self'][layernum]  * 0.5 \
                                            +    0.5* sim[h:] 

                        if inject:
                            sim[h:] = controller_for_inject[1].attention_store['up_self'][layernum]  * 0.5 \
                                            +    0.5 * sim[h:] 
