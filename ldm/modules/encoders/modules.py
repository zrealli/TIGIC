...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    #         # tokens[0] = torch.zeros(77)
    #         # tokens[0] = torch.zeros(77)+7788
    #         print(tokens[0]) 
    #         z = self.encode_with_transformer(tokens.to(self.device), inv=True)
    #         # print(z.shape)
    #     else:
    #         z = self.encode_with_transformer(tokens.to(self.device),inv=True)
    ...
    def forward(self, text, inv=False):
        ...
        if inv:    
            if tokens[0][1] == 49407:
                # tokens[0] = torch.zeros(77) 
                # print(tokens[0])
                z = self.encode_with_transformer(tokens.to(self.device), inv=False)
            else:
                # tokens[0] = torch.zeros(77)
                # print(tokens[0])
                z = self.encode_with_transformer(tokens.to(self.device), inv=False)
        else:
            # tokens[0] = torch.zeros(77) + 788 
