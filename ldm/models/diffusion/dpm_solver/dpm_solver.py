...
def model_wrapper(
    ...
    def model_fn(x, t_continuous, DPMencode=False, controller=None, inject=False):
        """
...
class DPM_Solver:
    ...
    def noise_prediction_fn(self, x, t, DPMencode=False, controller=None, inject=False):
        """
        Return the noise prediction model.
        """
        return self.model(x, t, DPMencode=DPMencode, controller=controller, inject=inject)

    def data_prediction_fn(self, x, t, DPMencode=False, controller=None, inject=False):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, t, DPMencode=DPMencode, controller=controller, inject=inject)
    ...
    def model_fn(self, x, t, DPMencode=False, controller=None, inject=False):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, DPMencode=DPMencode, controller=controller, inject=inject)
        else:
    ...
    def multistep_dpm_solver_second_update(self, x, model_prev_list, t_prev_list, t, solver_type="dpmsolver", DPMencode=False):
        """
    ...
    def multistep_dpm_solver_third_update(self, x, model_prev_list, t_prev_list, t, solver_type='dpmsolver', DPMencode=False):
        """
    ...
    def multistep_dpm_solver_update(self, x, model_prev_list, t_prev_list, t, order, solver_type='dpmsolver', DPMencode=False):
        """
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False, DPMencode=False
    ):
        """
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                step = 0
                t = timesteps[step].expand((x.shape[0]))
                t_prev_list = [t]
    ...
    def sample_one_step(self, data, step, steps, order=3, lower_order_final=True, solver_type='dpmsolver',
                        atol=0.0078, rtol=0.05, DPMencode=False, controller=None, inject=False):
        ...
        # We do not need to evaluate the final model value.
        if step < steps:
            data['model_prev_list'][-1] = self.model_fn(data['x'], vec_t, DPMencode=DPMencode, controller=controller, inject=inject)
        
        del vec_t
        
        return {'x': data['x'], 'model_prev_list': data['model_prev_list'], 't_prev_list': data['t_prev_list'], 'timesteps': data['timesteps']}
    
    def sample_lower(self, x, dpm_solver, steps, order, t_start, t_end, device, DPMencode=False, controller=None, inject=False):
        ...
        with torch.no_grad():
            ...
            model_prev_list = [dpm_solver.model_fn(x, vec_t, DPMencode=DPMencode, 
                                                   controller=[controller[0][0], controller[1][0]] if isinstance(controller, list) else controller, 
                                                   inject=inject)]
            ...
            for init_order in range(1, order):
                ...
                model_prev_list.append(dpm_solver.model_fn(x, vec_t, DPMencode=DPMencode, 
                                                           controller=[controller[0][init_order], controller[1][init_order]] if isinstance(controller, list) else controller,
                                                           inject=inject))
                total_controller.append(controller)
